...
/// Fork relationship metadata from a session's JSONL first entry
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ForkInfo {
...
/// Represents the first line of a JSONL session file (for extracting fork info)
#[derive(Debug, Deserialize)]
...
/// Extracts fork info from the first line of a JSONL session file.
/// Returns `Some(ForkInfo)` if the session was forked from another session.
fn extract_fork_info(jsonl_path: &PathBuf) -> Option<ForkInfo> {
...
/// Checks whether a user message content is a displayable human message
/// (not a system-generated tag or caveat).
fn is_displayable_user_message(content: &str) -> bool {
    if content.contains(
        "Caveat: The messages below were generated by the user while running local commands",
    ) {
        return false;
    }
    // Skip messages that start with XML-like tags (system/command messages)
                ...
                if let Some(message) = entry.message {
                    if message.role.as_deref() == Some("user") {
                        if let Some(content) = message.content {
                            if !is_displayable_user_message(&content) {
                                continue;
                            }
...
/// Extracts the last user message from a JSONL session file.
/// Scans the entire file to find the final valid user message.
fn extract_last_user_message(jsonl_path: &PathBuf) -> Option<String> {
    let file = match fs::File::open(jsonl_path) {
        Ok(file) => file,
        Err(_) => return None,
    };
    ...
    for entry in entries {
        ...
        if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("jsonl") {
            if let Some(session_id) = path.file_stem().and_then(|s| s.to_str()) {
                ...
                let last_message = extract_last_user_message(&path);
                ...
                let forked_from = extract_fork_info(&path);
